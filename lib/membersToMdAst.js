 ...
 * @ignore
 */
const membersToMdAst = (members, topDepth = 1) => {
    ...
    members
      ...
      .forEach(member => {
        ...
        if (member.description)
          mdast.children.push(
            ...mdToMdAst(member.description, outlinedMembers, false)
          )
        ...
        if (typeTag)
          mdast.children.push({
            ...
            children: [
              ...
              { type: 'text', value: ' ' },
              ...(typeTag.type ? typeJsdocAstToMdAst(typeTag.type, outlinedMembers) : {})
            ]
          })
        ...
        if (propTags) {
          ...
          propTags.filter(tag => tag.type).forEach(tag => {
        ...
        if (paramTags) {
          ...
          paramTags.filter(tag => tag.type).forEach(tag => {
            const typeCellChildren = typeJsdocAstToMdAst(
              tag.type,
              outlinedMembers
            )
